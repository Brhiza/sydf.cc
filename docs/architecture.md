...
面向用户与开发者的默认入口应保持“直接可用”：新增排盘参数只能作为可选高级能力，不应让普通用户或基础 API 调用方必须理解参数后才能使用。

`src/services/mingyu-core-regression/` 仅保留迁移到 `mingyu-core` 后的核心库回归测试，不属于运行时代码，也不是本地算法副本。

`src/services/daily-fortune/` 是站点侧今日运势组合逻辑。日家奇门排盘数据来自 `mingyu-core`，本仓库只组合评分、幸运元素和展示所需结构，不等同于 `mingyu-core` 的独立公开算法。
