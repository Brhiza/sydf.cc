...
function generateRandomCards() {
    const availableCards = [...tarotCards]; // 创建一个可修改的副本
    const selectedCards = [];
    for (let i = 0; i < 3; i++) {
        if (availableCards.length === 0) break; // 如果牌抽完了，提前结束
        const randomIndex = Math.floor(Math.random() * availableCards.length);
        const selectedCard = availableCards.splice(randomIndex, 1)[0]; // 从副本中抽取并移除
...
document.getElementById('submitButton').addEventListener('click', async () => {
    ...
    const imgElements = outputText.querySelectorAll('.lazy-load');
