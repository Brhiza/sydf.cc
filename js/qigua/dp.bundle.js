...
// dp-renderer.js
const imageUrlPrefix_dp = "../static/mx/";
function renderSingleTarotCard(cardData) {
    const { number, name, position } = cardData;
    ...
    outputText.innerHTML = `
        <div class="result-section">
            ...
            <div class="hexagram-container tarot">
                <div class="hexagram-item tarot">
                    <img src="${imageUrlPrefix_dp}${number}.jpg" alt="${displayName}" class="${reversedClass}" />
...
document.addEventListener('DOMContentLoaded', () => {
    const latest = window.loadLatestHistory('塔罗牌·单牌');
    if (latest) {
...
document.getElementById('submitButton').addEventListener('click', async () => {
    ...
    shuffleDeck();
    const [cardData] = drawCards(1);
    const displayName = cardData.position === 1 ? `逆位${cardData.name}` : cardData.name;
    const prompt = `你是一个塔罗牌大师，需要根据我抽到的牌和问题去解析，并解释卡牌的含义和解析，并在最后用百分比表示问题的概率，我抽到了${displayName}，我想知道 ${userInput}\n请帮我解读一下，并以如下格式回复\n此牌象意味着：\n你可以：`;
