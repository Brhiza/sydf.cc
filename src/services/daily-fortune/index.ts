...
export function calculateDailyFortune(date?: Date): DailyFortuneData {
  ...
  const dateStr = formatLocalDateKey(targetDate);

  const qimenData = generateMingyuDailyQimen(targetDate);
  const { ganzhi, isYangDun, juShu, timestamp, zhiFu, zhiShi } = qimenData;
  ...
  const dailyQimenTimeInfo: DailyQimenTimeInfo = {
    solarTerm: qimenData.timeInfo.solarTerm,
  ...
  const luckyElements = generateTraditionalLuckyElements(jiuGongGe, ganzhi, qimenData.directions);
