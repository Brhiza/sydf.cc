...
import { generatePromptAIResponse } from '../ai-client';
import { aiService } from '../aiService';
import { generateFollowUpPromptWrapper } from '../prompts';
import type {
  AIRegenerationRecord,
  GenerateRegeneratedAIOptions,
  RegeneratedAIResult,
} from './types';
import {
  buildRegeneratedConversationHistory,
  getChaoticLiuyaoResponse,
  getRegenerationQuestion,
} from './question-builder';

function cloneSupplementaryInfo(supplementaryInfo?: SupplementaryInfo) {
  return supplementaryInfo ? cloneSerializable(supplementaryInfo) : undefined;
}
...
export async function generateRegeneratedAI(
  ...
  if (chaoticResponse) {
    ...
    options.onConversationUpdate?.([...conversationHistory]);
    return { aiResponse: chaoticResponse, conversationHistory, target: 'primary' };
  }
  ...
  return { aiResponse, conversationHistory, target: 'primary' };
}
...
export async function regenerateConversationMessage(
  ...
  const { conversationHistory, targetMessage, targetIndex, isPrimaryTarget } =
    resolveTargetAssistantMessage(record, target);
  if (!targetMessage || targetMessage.role !== 'assistant' || targetIndex <= 0) {
    return generateRegeneratedAI(record, options);
  }
  if (isPrimaryTarget) return generateRegeneratedAI(record, options);
  ...
  const regeneratedAssistantMessage: ChatMessage = { ...targetMessage, content: '' };
