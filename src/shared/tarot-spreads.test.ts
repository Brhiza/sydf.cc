...
describe('tarot-spreads', () => {
  ...
  it('优先使用结果内已有牌阵名称', () => {
    expect(
      resolveTarotSpreadName({ spreadName: '自定义牌阵', spreadType: 'single', cardCount: 1 })
    ).toBe('自定义牌阵');
  });
  ...
  it('会安全解析牌阵配置', () => {
    expect(resolveTarotSpread('single')?.name).toBe('单牌指引');
    expect(resolveTarotSpread('unknown')).toBeUndefined();
    expect(resolveTarotSpread(null)).toBeUndefined();
  });
