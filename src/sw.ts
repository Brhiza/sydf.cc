...
interface WorkboxGlobalScope extends ServiceWorkerGlobalScope {
  workbox?: {
    setConfig: (config: { debug: boolean }) => void;
  };
}
...
const swSelf = self as WorkboxGlobalScope;
if (typeof swSelf !== 'undefined' && swSelf.workbox) {
  swSelf.workbox.setConfig({
    debug: false,
  });
}
